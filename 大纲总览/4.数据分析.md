...
│   │   ├── pd.read_json()
│   │   ├── json.load()
│   │   └── 将嵌套 JSON 转为 DataFrame：pd.DataFrame(data['users'])
│   │
│   └── 1.3 读取后的数据检查
│       ├── df.info()
│       └── df.tail()
...
│   ├── 数据清洗：dropna()
│   ├── 类型转换：sex → category
│   ├── 构造特征 bill_ratio＝长度/深度
│   ├── 分箱 body_mass_g → ['低','中','高']
...
└── 11. 综合案例：睡眠数据分析
    ...
    └── groupby 分组统计（age_level × bmi_category）